...
impl BftParams{
    fn new(program_file: PathBuf, tape_cell_count: Option<NonZeroUsize>, tape_is_extensible: bool) -> Self
    {
        Self{program_file,tape_cell_count, tape_is_extensible}
    }
}
...
fn run_bft(params: BftParams) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bf_interpreter: VirtualMachine<u8> = VirtualMachine::new(params.tape_cell_count,params.tape_is_extensible);
